...
public:
    // 构造函数
    explicit ThreadPool(int threadNum = DEFAULT_THREAD_NUM) : m_iThreadNum(threadNum), m_bStop(false), m_iTaskCount(0)
    {
        if (m_iThreadNum <= 0) {
        ...
        for (int i = 0; i < m_iThreadNum; ++i) {
            if (pthread_create(&m_piPthId[i], nullptr, work, this) != 0) {
                ...
                // 等待已创建的线程
                for (int j = 0; j < i; ++j) { pthread_join(m_piPthId[j], nullptr); }
    ...
    void shutdown()
    {
        ...
        // 等待所有线程结束
        for (int i = 0; i < m_iThreadNum; ++i) { pthread_join(m_piPthId[i], nullptr); }
    }
    ...
    void run()
    {
        while (true) {
...

            {
                ...
                // 等待任务或停止信号
                while (m_qTask.empty() && !m_bStop) { pthread_cond_wait(&m_cond, &m_mutex); }
...
int main()
{
    ...
    try {
        ...
        // 添加一些简单任务
        for (int i = 0; i < 5; ++i) { pool.addTask(testFunction, i); }

        // 添加带返回值的任务
        vector<future<int>> results;
        for (int i = 0; i < 3; ++i) { results.emplace_back(pool.addTask(testFunctionWithReturn, i + 10)); }
        ...
        // 获取返回值
        cout << "\n=== Results ===" << endl;
        for (auto& result : results) { cout << "Result: " << result.get() << endl; }
